...
func (builder *QueryBuilder) applyIndicesNew(nodeID int32) int32 {
	node := builder.qry.Nodes[nodeID]
	...
	default:
		for i, childID := range node.Children {
			node.Children[i] = builder.applyIndicesNew(childID)
		}
...
//	|
//
// table a join index table on pk join table b on a.id = b.id
func (builder *QueryBuilder) applyIndicesForJoin(nodeID int32, node *plan.Node) int32 {
	node.Children[1] = builder.applyIndicesNew(node.Children[1])

	leftChild := builder.qry.Nodes[node.Children[0]]
	if leftChild.NodeType != plan.Node_TABLE_SCAN {
		node.Children[0] = builder.applyIndicesNew(node.Children[0])
		return nodeID
