...
// tableScan(filter: inExpr)
//  |
//  |
//  |
//  tableScan join on pk indexTableScan(filter : inExpr)

// appleIndexForInExpression will try to use index which only has one column for in expression
func (builder *QueryBuilder) appleIndexForInExpression(nodeId int32, node *plan.Node) int32 {
	...
	for i, expr := range node.FilterList {
		...
		// builder filter for index table
